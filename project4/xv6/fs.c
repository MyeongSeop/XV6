...
void copy_swap(int past_blk, int new_blk)
{
	struct buf* bp;
	struct buf* past;
	int i;
	if ( new_blk < 0 || new_blk >= SWAPMAX/8 )
                panic("copy_swap: blkno exceed range");

	for(i=0;i<8;i++){
		//nr_sectors_write++;
		bp = bread(0, new_blk*8 + SWAPBASE + i);
		past = bread(0, past_blk*8 + SWAPBASE + i);
		memmove(bp->data, past->data, BSIZE);
		bwrite(bp);
		brelse(bp);
		brelse(past);
	}
...
void dealloc_swap(int blkno)
{
	
	if(blkno < 0 || blkno >= SWAPMAX/8)
		panic("dealloc_swap: blkno exceed range");
